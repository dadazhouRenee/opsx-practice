...
- [x] 2.2.1 在 `lib/theme/constants.ts` 中新增青色配置：cyan: { name: '青色', primary: '189 94% 43%', primaryForeground: '0 0% 100%' }
- [x] 2.2.2 在 `lib/theme/constants.ts` 中新增靛蓝色配置：indigo: { name: '靛蓝', primary: '239 84% 67%', primaryForeground: '0 0% 100%' }
...
- [x] 3.1.1 在 `app/globals.css` 中新增 `[data-theme="cyan"]` CSS 变量定义
- [x] 3.1.2 在 `app/globals.css` 中新增 `[data-theme="indigo"]` CSS 变量定义
...
- [x] 7.2.1 更新 ThemeSwitcher 组件的主色调选择器布局，从 2x2 网格改为 3x2 网格以容纳 6 种颜色（grid-cols-3）
...
- [x] 8.5.1 在组件预览 Card 的 Alert 区域添加主色 Alert：使用浅色背景样式（背景：bg-primary/10，边框：border-primary，图标和标题：text-primary），图标使用 CheckCircle2，标题"成功提示"，描述"这是一个主色调的提示消息，随主题颜色变化而变化。"
...
- [x] 9.2.1 测试新增的 6 种主题颜色切换（包括青色和靛蓝色），验证所有 primary 颜色的组件（按钮、Select focus、主色 Alert、选中边框）实时更新
- [x] 9.3 测试主题持久化：切换主题后刷新页面，验证主题偏好恢复
- [x] 9.4 测试 Sidebar 导航：点击"首页"和"设置"菜单项，验证路由跳转和激活状态高亮
- [x] 9.5 测试所有 8 种主题组合（4 色 × 2 模式），确保无视觉错误或样式异常
- [x] 9.5.1 测试所有 12 种主题组合（6 色 × 2 模式），确保无视觉错误或样式异常，特别关注主色 Alert 在不同主题下的显示效果
